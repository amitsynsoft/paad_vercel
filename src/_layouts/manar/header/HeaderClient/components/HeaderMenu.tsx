          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end h-full">
            <div className="flex flex-col gap-0 sm:gap-0 xl:gap-1">
              {headerData?.menuItems?.map((item, index) => (
                <Link key={index} href={item.url} onClick={() => setOpen(false)} className="text-[30px] md:text-[32px] 2xl:text-[40px] sentence-case font-semibold text-foreground hover:underline transition-all">
                  {item.label}
                </Link>
              ))}
            </div>
            ...
            <div className="md:w-[320px] md:h-[320px] relative md:block hidden">
              {/* Desktop / Tablet logo */}
              <Image src={mode === 'dark' ? (headerData?.headerLogo?.[0]?.dark?.src ?? '') : (headerData?.headerLogo?.[0]?.light?.src ?? '')} alt="Menu Logo" fill className="object-contain" />
            </div>
            <div className="relative md:hidden">
              {/* Mobile logo */}
              <Image src={mode === 'dark' ? (headerData?.mobileLogo?.[0]?.dark?.src ?? '') : (headerData?.mobileLogo?.[0]?.light?.src ?? '')} alt="Menu Logo Mobile" width={350} height={110} className="object-contain" />
            </div>
          </div>
        </div>
      </div>
